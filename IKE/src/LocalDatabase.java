...
/**
 * Class to create and manage a local SQLite database to use as a Last.FM-subset
 * databank
 * 
...
public class LocalDatabase {
	...
	protected void loadDriver() {
		try {
			...
			System.out
					.println("Can't load JDBC Driver.  Make sure classpath is correct");
		}
	}
	...
	protected void createTable(String query) {
		...
		try {
			...
			System.out
					.println("Something went horribly wrong creating tables\n");
	...
	/**
	 * Creates a table Persons and adds it to the current database if it wasn't
	 * already there Persons contain, last.fm-username, age, country, language &
	 * gender.
	 */
	protected void createTablePersons() {
		String query = "CREATE TABLE IF NOT EXISTS Persons ("
				+ "lfm_username	VARCHAR(128)," + "age 			INTEGER,"
				+ "country		VARCHAR(128)," + "language		VARCHAR(64),"
				+ "gender			VARCHAR(8)," + "PRIMARY KEY 	(lfm_username)" + ");";
		createTable(query);
	}

	/**
	 * Creates a table Recordings and adds it to the current database if it
	 * wasn't already there Recordings contains: artist name/track title
	 */
	protected void createTableRecordings() {
		String query = "CREATE TABLE IF NOT EXISTS Recordings ("
	...
	/**
	 * Creates a table Listenings and adds it to the current database if it
	 * wasn't already there Recordings contains: which user listens to which
	 * track
	 */
	protected void createTableListenings() {
		String query = "CREATE TABLE IF NOT EXISTS Listenings ("
				+ "user			VARCHAR(128),"
	 ...
	 * 
	 * @return TRUE iff name is indeed a table in the current database, FALSE
	 *         otherwise
	 */
	...
	/**
	 * Closes any open connection to the database, assuming the database isn't
	 * locked.
	 */
	public void closeConnection() {
		try {
			...
			System.out
					.println("ERROR: cannot close connection to local database");
		}
	}

	/**
	 * Inserts given data into persons table At this moment in time, still stubs
	 * :)
	 * 
	 * @param DBName
	 *            the name of the Database for which this method will insert
	 *            example data.
	 */
	protected void insertDataIntoPersons(Collection<User> users) {
		...
		try {
			...
			System.out
					.println("Something went wrong with inserting batched data into Persons");
	...
	protected void insertDataIntoRecordings(Collection<Track> tracks) {
		...
		try {
			PreparedStatement prep = conn
					.prepareStatement("INSERT INTO Recordings values (?, ?);");
			...
			System.out
					.println("Something went wrong with inserting batched data into Recordings");
			System.out.println("Check System.err for details");
			e.printStackTrace(System.err);
		}
	...
	protected void insertDataIntoListenings(User u, Collection<Track> tracks) {
		...
		try {
			PreparedStatement prep = conn
					.prepareStatement("INSERT INTO Listenings values (?, ?, ?);");

			for (Track t : tracks) {
				prep.setString(1, u.getName());
				prep.setString(2, t.getName());
				// DONNO IF THIS WORKS, LastFM Api doesnt show it but eclipse
				// does....
			...
			System.out
					.println("Something went wrong with inserting batched data into Listenings");
			System.out.println("Check System.err for details");
			e.printStackTrace(System.err);
		}
	...
	protected int countUsers() {
		...
		try {
			...
			ResultSet rs = stat
					.executeQuery("SELECT COUNT(lfm_username) FROM Persons;");
			result = rs.getInt(1);
		} catch (SQLException e) {
			System.out
					.println("Something went wrong counting users from Persons");
		}
	...
	/**
	 * Selects entries from Persons and returns a collection of Users complying
	 * with the demands made in where-statement
	 */
	protected Collection<RS_User> selectUsersFromPersons(String query) {
		...
		try {
			...
			while (rs.next()) {
				...
				RS_User rsu = new RS_User(username, age, country, language,
						gender);
				users.add(rsu);
			}
		} catch (SQLException e) {
			System.out
					.println("Something went wrong selecting results from Persons");
		}
		return users;
	}

	/**
	 * Selects entries from Recordings and returns a collection of Tracks
	 * complying with the demands made in where-statement
	 */
	protected Collection<RS_Track> selectTracksFromRecordings(String query) {
		...
		try {
			...
			while (rs.next()) {
				...
				// TODO overleg met Thomas/Robbert welke info hier
				// opslaan/ophalen??
			}
		} catch (SQLException e) {
			System.out
					.println("Something went wrong selecting results from Recordings");
		}
		return tracks;
	}
	...
	public Collection<RS_Track> getTracks(String user) {
		// TODO
		String query = "SELECT artist_name, track_name FROM Recordings, Listenings "
	...
	public Collection<RS_User> getListeners(Track t) {
		// TODO
		String query = "SELECT user.lfm_username AS lfm_username, age, country, language, gender FROM Persons, Listenings, Recordings "
	...
	/**
	 * Set up a basic local database ready for use. Just open connection on this
	 * one and ready to go! Loads driver, opens connection to make the tables,
	 * closes connection = done ;)
	 * */
	...
	/**
	 * Method that fills the database. Takes a track and gets all top listeners
	 * form that track. Then it fills the listenings table for each of those
	 * users. Then it fills recordings with the information of all tracks in
	 * database.
	 * */
			...
			for (User u : users) {
				if (limit > 0) {
					if (u != null) {
						limit--;
						try {
							Collection<Track> tracks = User.getTopTracks(u
									.getName(), key);
							...
							System.out.println(u.getName()
									+ "s' tracks added to recordings");
						} catch (Exception e) {
							System.out
									.println("Invalid Used data while adding to listeners");
						}
					}
				}
			}
